...
// Gamitin ang nurse_id mula sa session (tulad ng ginagawa sa nurse_queue.php)
...
// Handle Start Consultation (from appointments to queue)
if (isset($_GET['start_consultation']) && isset($_GET['id'])) {
    $appointment_id = intval($_GET['id']);
    
    // Kunin ang appointment details
    $apt_stmt = mysqli_prepare($conn, "SELECT student_id, symptoms, purpose FROM appointments WHERE id = ?");
    ...
    if ($appointment) {
        // Check kung may existing visit na waiting/in-progress for this student
        $check_stmt = mysqli_prepare($conn, "SELECT id FROM visits WHERE student_id = ? AND status IN ('waiting', 'in-progress')");
        mysqli_stmt_bind_param($check_stmt, "i", $appointment['student_id']);
        mysqli_stmt_execute($check_stmt);
        $check_result = mysqli_stmt_get_result($check_stmt);
        
        if (mysqli_num_rows($check_result) > 0) {
            // May existing visit, i-update na lang ang appointment status to completed (para mawala sa list)
            $upd_stmt = mysqli_prepare($conn, "UPDATE appointments SET status = 'completed' WHERE id = ?");
            mysqli_stmt_bind_param($upd_stmt, "i", $appointment_id);
            mysqli_stmt_execute($upd_stmt);
            
            // Redirect sa queue page
            header('Location: nurse_queue.php');
            exit();
        } else {
            // Insert sa visits table - gumamit ng NULL sa nurse_id para iwas foreign key error
            $visit_stmt = mysqli_prepare($conn, "INSERT INTO visits (student_id, nurse_id, symptoms, diagnosis, status, visit_date) VALUES (?, NULL, ?, ?, 'waiting', NOW())");
            $diagnosis = "Appointment: " . ($appointment['purpose'] ?? 'Consultation');
            mysqli_stmt_bind_param($visit_stmt, "iss", $appointment['student_id'], $appointment['symptoms'], $diagnosis);
            
            if (mysqli_stmt_execute($visit_stmt)) {
                // Kunin ang bagong visit ID
                $new_visit_id = mysqli_insert_id($conn);
                
                // Update appointment status to completed (para mawala sa list)
                $upd_stmt = mysqli_prepare($conn, "UPDATE appointments SET status = 'completed' WHERE id = ?");
                mysqli_stmt_bind_param($upd_stmt, "i", $appointment_id);
                mysqli_stmt_execute($upd_stmt);
                
                // Create notification
                createNotification($appointment['student_id'], 'appointment', 'Appointment Started', "Your appointment has been started and added to the queue.", 'student_appointments.php');
...
// Handle Complete (from appointments to queue)
if (isset($_GET['complete_to_queue']) && isset($_GET['id'])) {
    $appointment_id = intval($_GET['id']);
    ...
    if ($appointment) {
        ...
        if (mysqli_num_rows($check_result) > 0) {
            // May existing visit, i-update lang ang status nito to completed
            $row = mysqli_fetch_assoc($check_result);
            $visit_id = $row['id'];
            
            $update_visit = mysqli_prepare($conn, "UPDATE visits SET status = 'completed', consultation_date = NOW() WHERE id = ?");
            mysqli_stmt_bind_param($update_visit, "i", $visit_id);
            mysqli_stmt_execute($update_visit);
            ...
            // Create notification
            createNotification($appointment['student_id'], 'appointment', 'Appointment Completed', "Your appointment has been completed.", 'student_appointments.php');
            
            // Redirect sa queue page
            header('Location: nurse_queue.php');
            exit();
        } else {
            // Walang existing visit, gumawa ng completed visit record gamit ang NULL sa nurse_id
            $visit_stmt = mysqli_prepare($conn, "INSERT INTO visits (student_id, nurse_id, symptoms, diagnosis, status, visit_date, consultation_date) VALUES (?, NULL, ?, ?, 'completed', NOW(), NOW())");
            $diagnosis = "Appointment: " . ($appointment['purpose'] ?? 'Consultation');
            mysqli_stmt_bind_param($visit_stmt, "iss", $appointment['student_id'], $appointment['symptoms'], $diagnosis);
...
// Handle status update (for Confirm and Cancel only)
if (isset($_GET['update_status']) && isset($_GET['id'])) {
    ...
    if ($appointment) {
        ...
        // I-update ang status (for confirm and cancel only)
        ...
        if (mysqli_stmt_execute($stmt)) {
            ...
            // Redirect back with filters - para ma-refresh ang list at mawala ang cancelled/confirmed
                                    ...
                                    <td class="px-5 py-3">
                                        <div class="flex flex-wrap gap-2">
                                            ...
                                            <?php elseif ($apt['status'] == 'confirmed'): ?>
                                                ...
                                                <?php else: ?>
                                                    <a href="?start_consultation=1&id=<?php echo $apt['id']; ?>" 
                                                       onclick="return confirm('Start consultation for this patient?')"
                                                ...
                                                <?php endif; ?>
                                                <a href="?complete_to_queue=1&id=<?php echo $apt['id']; ?>" 
                                                   class="text-gray-600 hover:text-gray-800 text-xs font-medium bg-gray-50 px-2 py-1 rounded"
                                                   onclick="return confirm('Mark as completed and add to queue?')">
            ...
            <div class="mt-6 bg-blue-50 border border-blue-200 rounded-xl p-4">
                <div class="flex items-start gap-3">
                    ...
                    <div class="text-sm text-blue-800">
                        ...
                        • <strong>Other purposes:</strong> Click "Complete" to mark as done and add to the queue.<br>
                    </div>
                </div>
            </div>
        </div>
